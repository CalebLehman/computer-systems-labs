...
phase, each on a separate line. Descriptions of the methods used to find
...
Disassembling `phase_1`, we see that it just compares the input against
the string at `0x080497c0`. Examining this memory with `x/s` yields
`Public speaking is very easy.`

## Phase 2

Disassembling `phase_2`, we see that it starts by calling
`read_six_numbers` on the input.
...
Examining this memory with `x/s` yields `"%d %d %d %d %d %d"`,
so the input should be six integers.

Back inside `phase_2`,
the arguments of `read_six_numbers` are the input and the location
`$ebp-0x18` on the stack, where the six integers are stored.
The first integers (at `$ebp-0x18`) checked against `0x1`.
...
and a loop index (`$ebx + 1`), that is, our password `a[1] ... a[6]`
```
a[1] = 1
a[i] = i * a[i-1]
...
parsed out. Examining the memory with `x/s`, we see that the format
...
integer. We can define a short `gdb` command to print the passwords:
```
break *phase_3+62
...
are many valid passwords. For fun, we can easily generate a random ascii
password with a short `python` script.
...
for i, c in enumerate(cipher_text):
    ...
    def validator(char):
        try:
            ...
            return ascii_char.isalnum()
        except UnicodeEncodeError:
            return False
    possible_chars = (chr(lo+(hi<<4)) for hi in range(16))
    valid_chars    = filter(validator, possible_chars)
    plain_text_chars.append(choice(list(valid_chars)))
...
As mentioned earlier, the symbol contains an entry for the function
...
performs the check `i - 1 <= 1000`. The conditional jump is implemented
...
space is so small, we might try simply checking every result with a
simple `gdb` script:
```
...
r < [input-file] > /dev/null
```
This produces `1001` as the password. For fun, we can investigate `fun7`
...
With this in mind, we see that for any `input` that is actually present
in the tree, `fun7` computes the binary number obtained by taking the
...
don't handle recursion well, due to the way it handles variables)
```
import gdb
